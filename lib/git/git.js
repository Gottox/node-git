...
var Git = exports.Git = function(git_directory) {
  var _git_directory = git_directory, _git_file_index;
  var _repository = new Repository(_git_directory, {});
...
Git.prototype.call_git = function(prefix, command, gitargs, postfix, options, args, callback) {
  ...
  if(process.platform.toLowerCase().match(/mswin(?!ce)|mingw|bccwin/)) {
    ...
    // Transform and map the git parameters
    var ext_git_args = this.transform_options(gitargs);
    if (ext_git_args.length) {
      ext_git_args += ' ';
    }
    // Join the arguments
    var final_arguments = option_arguments.concat(ext_args);
    // Build a call
    call_string = prefix + Git.git_binary + ' --git-dir="' + shell_escape(this.git_directory) + '" ' + ext_git_args  + command.toString().replace(/_/, '-') + ' ' + final_arguments.join(" ") + postfix;
  }
  // Execute the function
  this.exec(call_string, { encoding: 'utf8', timeout: timeout, killSignal: 'SIGKILL'}, callback);
}

// Necessary for testing
Git.prototype.exec = function(call_string, options, callback) {
